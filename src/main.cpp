...
void signal_handler(int signal) {
    BOOST_LOG_TRIVIAL(info) << "Received signal " << signal << ", shutting down...";
    if (g_server) {
        g_server->stop();
    }
    exit(0);
...
void print_usage() {
    std::cout << "Usage: open-ctp-mdgateway [options]" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    BOOST_LOG_TRIVIAL(info) << "  QuantAxis Market Data Server";
    BOOST_LOG_TRIVIAL(info) << "========================================";
    ...
    try {
        if (use_multi_ctp) {
            ...
            if (!config_file.empty()) {
                ...
                // 使用默认多CTP配置
                BOOST_LOG_TRIVIAL(info) << "Using default multi-CTP configuration (SimNow)";
            ...
            if (!ConfigLoader::validate_config(config)) {
                BOOST_LOG_TRIVIAL(error) << "Invalid configuration";
                return 1;
            }
            ...
            for (const auto& conn : config.connections) {
                if (conn.enabled) {
                    BOOST_LOG_TRIVIAL(info) << "    [" << conn.connection_id << "] " << conn.front_addr 
                                            << " (priority: " << conn.priority 
                                            << ", max_subs: " << conn.max_subscriptions << ")";
                }
            }
            ...
            // 单CTP连接模式（兼容性）
            BOOST_LOG_TRIVIAL(info) << "Single-CTP Mode Configuration:";
            BOOST_LOG_TRIVIAL(info) << "  MD Front:     " << front_addr;
            BOOST_LOG_TRIVIAL(info) << "  Broker ID:    " << broker_id;
            BOOST_LOG_TRIVIAL(info) << "  WebSocket:    ws://0.0.0.0:" << port;
            BOOST_LOG_TRIVIAL(info) << "  Auth:         No credentials required for market data";
            BOOST_LOG_TRIVIAL(info) << "========================================";
        ...
        if (show_status) {
            if (!g_server->start()) {
                BOOST_LOG_TRIVIAL(error) << "Failed to start server for status check";
                return 1;
            }
            ...
            BOOST_LOG_TRIVIAL(info) << "Connection Status:";
            BOOST_LOG_TRIVIAL(info) << "  Active connections: " << g_server->get_active_connections_count();
            BOOST_LOG_TRIVIAL(info) << "  CTP connected: " << (g_server->is_ctp_connected() ? "Yes" : "No");
            BOOST_LOG_TRIVIAL(info) << "  CTP logged in: " << (g_server->is_ctp_logged_in() ? "Yes" : "No");
        ...
        BOOST_LOG_TRIVIAL(info) << "Server started successfully.";
        BOOST_LOG_TRIVIAL(info) << "WebSocket endpoint: ws://localhost:" << port;
        BOOST_LOG_TRIVIAL(info) << "Press Ctrl+C to stop.";
        ...
        while (g_server->is_running()) {
            ...
            if (use_multi_ctp && g_server->get_connection_manager()) {
                ...
                BOOST_LOG_TRIVIAL(info) << "[Status] Active connections: " << active_conns 
    ...
    BOOST_LOG_TRIVIAL(info) << "Server stopped gracefully.";
    shutdown_logging();
    return 0;
}